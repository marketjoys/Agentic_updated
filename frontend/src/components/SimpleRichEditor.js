...
import { Bold, Italic, Underline, List, Link, Type, AlignCenter, Code } from 'lucide-react';

const SimpleRichEditor = forwardRef(({ value, onChange, placeholder, onKeyDown, className, ...props }, ref) => {
    ...
    <div className={className}>
      ...
      <div className="border border-gray-300 rounded-t-lg bg-gray-50 px-3 py-2 flex flex-wrap gap-1">
        {formatButtons.map((button, index) => (
          <button
            ...
            title={button.title}
            className={`p-2 rounded hover:bg-gray-200 transition-colors ${isHtmlMode && button.icon === Code ? 'bg-blue-100' : ''}`}
          >
      ...
      <div className="relative">
        <textarea
          ...
          placeholder={placeholder || 'Enter your email content here... Use {{variable_name}} for personalization'}
